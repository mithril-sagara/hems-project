    ...
    try:
        ...
        for table in tables:
            for r in table.records:
                bk = r.values.get("buy", 0); sk = r.values.get("sell", 0)
...
def api_history():
    ...
    try:
        ...
        if u == "day":
            ...
            coeff = COEFF_MAP.get(m_num, 1.5)
            forecast_list = [round(min((w / 1000.0) * PANEL_CAPACITY * coeff, PANEL_CAPACITY), 2) for w in w_res['hourly']['shortwave_radiation']]
        elif u == "month":
            for d in range(len(labels)):
                day_slice = w_res['hourly']['weather_code'][d*24:(d+1)*24]
                if day_slice: weather_list[d] = W_MAP.get(max(set(day_slice), key=day_slice.count), "-")
                rad_slice = w_res['hourly']['shortwave_radiation'][d*24:(d+1)*24]
                if rad_slice: radiation_list[d] = round(sum(rad_slice)/1000.0, 2)
    except: pass
...
def get_devices():
    return jsonify(requests.get("https://api.switch-bot.com/v1.1/devices", headers=sb_headers()).json().get("body", {}))
...
def index():
    ...
    <style>
        ...
        .area-live { height: 280px; position: relative; background: #000; border-bottom: 2px solid #334155; }
        ...
        .node.grid { top: 130px; left: 40px; border-color: var(--buy); }
        .node.home { top: 130px; right: 40px; border-color: var(--home); }
        .solar-total { position: absolute; top: 50px; left: calc(50% + 50px); font-size: 15px; font-weight: bold; color: #fff; }
        ...
        <div id="left">
            <div class="area-live">
                ...
                <div class="acc-info">
                    ...
                    <div style="font-size:15px;">売電: <span id="v-sk" style="color:var(--sell)">0</span>kWh (<span id="v-sy">0</span>円)</div>
                </div>
            </div>
            <div class="area-data">
